...
open class GrowingTextView: UITextView {
    ...
    @IBInspectable open var customPlaceholderColor: UIColor = UIColor(white: 0.8, alpha: 1.0) {
        didSet { setNeedsDisplay() }
    }
    @IBInspectable open var customAttributedPlaceholder: NSAttributedString? {
        didSet { setNeedsDisplay() }
    }
    ...
    override open func layoutSubviews() {
        ...
        if (customHeightConstraint == nil) {
            customHeightConstraint = NSLayoutConstraint(item: self, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: height)
            addConstraint(customHeightConstraint!)
        }
        
        // Update height constraint if needed
        if height != customHeightConstraint!.constant {
    ...
    override open func draw(_ rect: CGRect) {
        ...
        if text.isEmpty {
            ...
            if let attributedPlaceholder = customAttributedPlaceholder {
                // Prefer to use attributedPlaceholder
                attributedPlaceholder.draw(in: placeholderRect)
            } else if let placeholder = customPlaceholder {
